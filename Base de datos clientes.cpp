...
//INSERTAR RENTAS(ARCHIVO PARA RENTAS DONDE ESTEN LOS NIT DE LOS CLIENTES)
void menu::insertarRentas()
{
    ...
    if(escritura.is_open() && verificador.is_open())
    {
        cout<<"\t\t\t\t***Ingresar Renta***\t\t\t\t\n\n";
        fflush(stdin);
        cout<<"Ingresa el c\242digo de Renta: ";
        getline(cin,auxCodigo);
        if(auxCodigo=="")
            do
            {
                cout<<"c\242digo de Renta no v\240lido!, intentalo nuevamente: ";
                getline(cin,auxCodigo);
            }
            while(auxCodigo=="");
        do
        {
            verificador.seekg(0);
            getline(verificador,codigoRenta);
            while(!verificador.eof())
            {
                getline(verificador,nombre);
                getline(verificador,domicilio);
